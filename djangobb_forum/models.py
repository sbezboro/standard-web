...
class Category(models.Model):
    ...
    @property
    def topics(self):
        return Topic.objects.filter(deleted=False, forum__category__id=self.id).select_related()

    @property
    def posts(self):
        return Post.objects.filter(deleted=False, topic__forum__category__id=self.id).select_related()
...
class Forum(models.Model):
    ...
    @property
    def posts(self):
        return Post.objects.filter(deleted=False, topic__forum__id=self.id).select_related()
...
class Topic(models.Model):
    ...
    def delete(self, *args, **kwargs):
        ...
        try:
            forum.last_post = Topic.objects.filter(deleted=False, forum__id=forum.id).latest().last_post
        except Topic.DoesNotExist:
            forum.last_post = None
        forum.topic_count = Topic.objects.filter(deleted=False, forum__id=forum.id).count()
        forum.post_count = Post.objects.filter(deleted=False, topic__forum__id=forum.id).count()
        forum.save()
...
class Post(models.Model):
    ...
    def delete(self, *args, **kwargs):
        ...
        if self_id == head_post_id:
            ...
            try:
                topic.last_post = Post.objects.filter(deleted=False, topic__id=topic.id).latest()
            except Post.DoesNotExist:
                topic.last_post = None
            topic.post_count = Post.objects.filter(deleted=False, topic__id=topic.id).count()
            topic.save()
        try:
            forum.last_post = Post.objects.filter(deleted=False, topic__forum__id=forum.id).latest()
        except Post.DoesNotExist:
            forum.last_post = None
        ...
        forum.topic_count = Topic.objects.filter(deleted=False, forum__id=forum.id).count()
        forum.save()
        profile.post_count = Post.objects.filter(deleted=False, user__id=self.user_id).count()
        profile.save()
...
class Profile(models.Model):
    ...
    post_count = models.IntegerField(_('Post count'), blank=True, default=0)
    player = models.ForeignKey('standardweb.MinecraftPlayer', null=True, related_name='djangobb_profile')
